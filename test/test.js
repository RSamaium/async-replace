...
test('async-replace', function(t) {
    ...
    t.test('simple local', async function(t) {
        const newString = await asyncReplace(' foo ', /(fo)(.)/, function(match, p1, p2, offset, input, callback) {
            t.equal(match, 'foo');
            t.equal(p1, 'fo');
            t.equal(p2, 'o');
            t.equal(offset, 1);
    ...
    t.test('simple global', async function(t) {
        const newString = await asyncReplace(' foo ', /(fo)(.)/g, function(match, p1, p2, offset, input, callback) {
            t.equal(match, 'foo');
            t.equal(p1, 'fo');
            t.equal(p2, 'o');
            t.equal(offset, 1);
    ...
    t.test('messy global', async function(t) {
        ...
        var offsets = [1, 5];
        var p2s = ['o', 'z'];
        const newString = await asyncReplace('1foo2foz3', /(fo)(.)/g, function(match, p1, p2, offset, input, callback) {
    ...
    t.test('global and ignoreCase', async function(t) {
        const newString = await asyncReplace(' Foo foo ', /(f)oo/gi, function(match, p1, offset, input, callback) {
            process.nextTick(function() {
                callback(null, p1);
            });
        });
    ...
    t.test('local and ignoreCase', async function(t) {
        const newString = await asyncReplace(' Foo foo ', /(f)oo/i, function(match, p1, offset, input, callback) {
            process.nextTick(function() {
                callback(null, p1);
            });
        });
